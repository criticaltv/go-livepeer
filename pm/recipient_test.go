...
func newTicket(sender ethcommon.Address, params *TicketParams, senderNonce uint32) *Ticket {
	return &Ticket{
		...
		CreationRound:          params.ExpectedExpirationParams.CreationRound,
		CreationRoundBlockHash: params.ExpectedExpirationParams.CreationRoundBlockHash,
	}
}
...
func genRecipientRand(sender ethcommon.Address, secret [32]byte, params *TicketParams) *big.Int {
	...
	msg = append(
		msg,
		append(
			ethcommon.LeftPadBytes(big.NewInt(params.ExpectedExpirationParams.CreationRound).Bytes(), uint256Size),
			params.ExpectedExpirationParams.CreationRoundBlockHash.Bytes()...,
		)...,
	)
	h.Write(msg)
...
func TestTicketParams(t *testing.T) {
	...
	assert.Equal(params1.ExpectedExpirationParams.CreationRound, tm.round.Int64())
	assert.Equal(params1.ExpectedExpirationParams.CreationRoundBlockHash.Bytes(), tm.blkHash[:])
