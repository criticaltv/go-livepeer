		...
		*newSess = *sess
		...
		for i, url := range segURLs {
			...
			if err != nil {
				...
				glog.Errorf("Playlist insertion error; is this recoverable? nonce=%d manifestID=%s seqNo=%d err=%s", nonce, cxn.mid, seg.SeqNo, err)
				return nil
			}
		}
	...
	if accepted != nil {
		...
		if accepted != params && !sess.BroadcasterOS.IsExternal() {
			for i, data := range accepted.Renditions {
				...
				// XXX fix required base path
				_, err := sess.BroadcasterOS.SaveData(path.Base(accepted.URIs[i]), data)
				if err != nil {
					return err
				}
